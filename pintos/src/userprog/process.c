...
//   /* After finishing mapping, wake up */
//   //메모리 적재 완료 시 부모 프로세스 다시 진행 using semaphore
...
//   //   for(nt = list_front(child_list);nt != list_end(child_list);nt = list_next(nt)){
...
//   //   }
//   // }
//   if(child_pcb == NULL){//NOT found
//     //printf("wait() : child not found, pid = %d\n",child_tid);
//     return -1;
//   }
//   if(child_pcb->waiting){//already waiting
//     //printf("wait() : child found, but it is already 'waiting', pid = %d\n",child_tid);
//     return -1;
//   }
//   else{
...
//   //Unblock the waiting parent process from wait(), and then free the resources
//   cur->pcb->exited = true;
...
//                                   //file_page : file의 위치
//                                   //mem_page : 파일을 저장할 메모리의 virtual address
//                  goto done;
//             }
//           else
//             goto done;
...
//       uint8_t *kpage = vn_frame_alloc (PAL_USER);
//       if (kpage == NULL)
//         return false;
...
//   /*
//   int i;
//   strlcpy(cmd,string,strlen(string)+1);
...
//   //Currently, string includes command and arguments.
//   //So we need to parse in this function
//   int i =0;
//   int len = 0;
//   void* addrs[*argc];

//   //save the address of argv
//   //push argv to stack
//   for(i = *argc - 1;i >= 0;i--){
//     len = strlen(argv[i]) + 1;
//     *esp -= len;
